...
// Test 4b: Environment variable takes priority
console.log('Test 4b: Environment profile priority');
try {
  const originalEnvProfile = process.env.SSH_MANAGER_PROFILE;
  process.env.SSH_MANAGER_PROFILE = 'frappe';
  assert(getActiveProfileName() === 'frappe', 'Environment profile should have priority');
  const envProfile = loadProfile();
  assert(envProfile.name === 'frappe', 'loadProfile without args should use active environment profile');
  if (originalEnvProfile === undefined) {
    delete process.env.SSH_MANAGER_PROFILE;
  } else {
    process.env.SSH_MANAGER_PROFILE = originalEnvProfile;
  }
  console.log('✅ Environment profile priority works\n');
} catch (error) {
  delete process.env.SSH_MANAGER_PROFILE;
  console.error(`❌ Failed environment profile priority: ${error.message}\n`);
  process.exit(1);
}
...
// Test 6b: Reject switching to non-existent profile
console.log('Test 6b: Reject invalid active profile');
try {
  const switchResult = setActiveProfile('non-existent-profile');
  assert(switchResult === false, 'Should reject missing profile');
  console.log('✅ Invalid profile switch rejected\n');
} catch (error) {
  console.error(`❌ Failed invalid profile switch test: ${error.message}\n`);
  process.exit(1);
}
...
// Test 8: Invalid profile JSON falls back safely and list skips it
console.log('Test 8: Invalid profile JSON handling');
try {
  const profilesDir = path.join(__dirname, '..', 'profiles');
  const invalidProfilePath = path.join(profilesDir, 'unit-invalid.json');
  fs.writeFileSync(invalidProfilePath, '{ invalid json');
  try {
    const profile = loadProfile('unit-invalid');
    assert(profile.name === 'default' || profile.name === 'minimal',
      'Invalid profile JSON should fall back safely');
    const profiles = listProfiles();
    assert(!profiles.some(p => p.file === 'unit-invalid.json'),
      'Invalid profile JSON should be skipped when listing profiles');
  } finally {
    fs.unlinkSync(invalidProfilePath);
  }
  console.log('✅ Invalid profile JSON handled safely\n');
} catch (error) {
  console.error(`❌ Invalid profile JSON handling failed: ${error.message}\n`);
  process.exit(1);
}
